...
const createActivePreview = (pathArray) => {
    ...
    const activePreview = {
        ...
        add: () => {
            ...
            node.innerHTML = `
            <div class="overlay__preview"><img class="overlay__blur" data-list-blur src="${active.image}"/><img class="overlay__image" data-list-image src="${active.image}"/></div>
            <div class="overlay__content">
            <h3 class="overlay__title" data-list-title>${active.title}</h3>
            <div class="overlay__data" data-list-subtitle>${authors[active.author]} (${new Date(active.published).getFullYear()}</div>
            <p class="overlay__data overlay__data_secondary" data-list-description>${active.description}</p>
            </div>
...
export const handleOpenActivePreview = (event) => {
    ...
    activePreview.image = 'https://www.catherinewickham.co.za/images/profilePicLarge.jpg'; // check setter
    activePreview.add()
}
